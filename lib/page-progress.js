...
const PageProgress = Class({
    ...
    initialize: function(browser, options) {
        ...
        this.browser.addProgressListener(this.progress, Ci.nsIWebProgress.NOTIFY_ALL);
        ...
        this.on('statechange', function(progress, request, flags, status, isMain) {
            ...
            if (this.isStart(flags)) {
                ...
                emit(this, 'contentloaded', !request.status, request.URI.spec);
            }
            else if (this.isLoaded(flags)) {
                emit(this, 'loadfinished', !request.status, request.URI.spec);
            }
        });
...
ProgressListener.prototype = {
    ...
    onStatusChange: function(progress, request, status, message) {
        ...
        emit(this.target, 'statuschange',
            progress, request, status, message, this._isFromMainWindow(progress, request)
        );
    },
